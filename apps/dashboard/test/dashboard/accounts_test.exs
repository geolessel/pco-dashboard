defmodule Dashboard.AccountsTest do
  ...
  describe "oauth_tokens" do
    alias Dashboard.Accounts.OauthToken

    @valid_attrs %{access_token: "some access_token", expires_at: ~N[2010-04-17 14:00:00], refresh_token: "some refresh_token"}
    @update_attrs %{access_token: "some updated access_token", expires_at: ~N[2011-05-18 15:01:01], refresh_token: "some updated refresh_token"}
    ...
    test "list_oauth_tokens/0 returns all oauth_tokens" do
      ...
      assert Accounts.list_oauth_tokens() == [oauth_token]
    end

    test "get_oauth_token!/1 returns the oauth_token with given id" do
      oauth_token = oauth_token_fixture()
      assert Accounts.get_oauth_token!(oauth_token.id) == oauth_token
    end

    test "create_oauth_token/1 with valid data creates a oauth_token" do
      assert {:ok, %OauthToken{} = oauth_token} = Accounts.create_oauth_token(@valid_attrs)
      assert oauth_token.access_token == "some access_token"
      assert oauth_token.expires_at == ~N[2010-04-17 14:00:00]
    ...
    test "create_oauth_token/1 with invalid data returns error changeset" do
      assert {:error, %Ecto.Changeset{}} = Accounts.create_oauth_token(@invalid_attrs)
    end

    test "update_oauth_token/2 with valid data updates the oauth_token" do
      oauth_token = oauth_token_fixture()
      assert {:ok, %OauthToken{} = oauth_token} = Accounts.update_oauth_token(oauth_token, @update_attrs)
      assert oauth_token.access_token == "some updated access_token"
      assert oauth_token.expires_at == ~N[2011-05-18 15:01:01]
    ...
    test "update_oauth_token/2 with invalid data returns error changeset" do
      oauth_token = oauth_token_fixture()
      assert {:error, %Ecto.Changeset{}} = Accounts.update_oauth_token(oauth_token, @invalid_attrs)
      assert oauth_token == Accounts.get_oauth_token!(oauth_token.id)
    end

    test "delete_oauth_token/1 deletes the oauth_token" do
      oauth_token = oauth_token_fixture()
      assert {:ok, %OauthToken{}} = Accounts.delete_oauth_token(oauth_token)
      assert_raise Ecto.NoResultsError, fn -> Accounts.get_oauth_token!(oauth_token.id) end
    end

    test "change_oauth_token/1 returns a oauth_token changeset" do
      oauth_token = oauth_token_fixture()
      assert %Ecto.Changeset{} = Accounts.change_oauth_token(oauth_token)
    end
  end
end
